/**
 * jQuery slicebox plugin http://www.codrops.com/
 * 
 * Copyright 2011, Pedro Botelho Licensed under the MIT license.
...
(function(window, $, undefined) {
	...
	$.slicebox.defaults = {
		...
		slicesCount : 1, // needs to be an odd number 15 => number > 0 (if
							// you want the limit higher, change the _validate
							// function).
		disperseFactor : 0, // each slice will move x pixels left / top
							// (depending on orientation). The middle slice
							// doesn't move. the middle slice's neighbors will
							// move disperseFactor pixels.
		colorHiddenSides : '#222', // color of the hidden sides.
		sequentialRotation : false, // the animation will start from left to
									// right. The left most slice will be the
									// first one to rotate.
		sequentialFactor : 0, // if sequentialRotation is true this will be
								// the interval between each rotation in ms.
		speed3d : 800, // animation speed3d.
		speed : 600, // fallback speed. You might want to set a different
						// speed for the fallback animation...
		...
		showInfo : false, // Show the info automatically
		prefx : '-webkit' // Used prefix for browser compatibility
	};
		...
		_validate : function(options) {
			if (Modernizr.csstransforms3d && navigator.userAgent.indexOf("Firefox/10") == -1 ) {
				if (navigator.userAgent.indexOf("Firefox") != -1) {
					// hope this will work better in later version (Firefox 10+)
					options.prefx = '-moz';
				...
				/*
				 * remove these three lines if you don't want to limit the
				 * number of slices (not sure if it works fine..)
				 */
				if (options.slicesCount < 0) {
					options.slicesCount = 1;
				} else if (options.slicesCount > 15) {
					options.slicesCount = 15;
				} else if (options.slicesCount % 2 === 0) {
					options.slicesCount++;
				}
			...
			if (options.orientation !== 'v' && options.orientation !== 'h') {
	...
	$.Box3d.prototype = {
		_createBox : function(options) {
			...
			boxStyle[options.prefx + '-perspective'] = options.perspective;
			...
			for ( var i = 0; i < options.slicesCount; ++i) {
				...
				var Slice3d = new $.Slice3d(options, this.size, this.orientation), $slice = Slice3d.createSlice(options, i, this.$images);
		...
		_configureSlices : function(options) {
			var middlepos = Math.ceil(options.slicesCount / 2), instance = this;

			for ( var i = 0, len = instance.slices.length; i < len; ++i) {
				var sliceObj = instance.slices[i], $slice = sliceObj.getEl();
	...
	$.Slice3d.prototype = {
		_configureStyles : function(options) {
			...
			this.style[options.prefx + '-transform-style'] = 'preserve-3d';
			this.style[options.prefx + '-transition'] = options.prefx + '-transform ' + options.speed3d + 'ms';
			this.style[options.prefx + '-backface-visibility'] = 'hidden';

			if (this.orientation === 'v') {
				this.animationStyles = {side1:{},side2:{},side3:{},side4:{}};
				this.animationStyles.side1[options.prefx + '-transform'] = 'translate3d( 0, 0, -' + (this.size.height / 2) + 'px )';
				this.animationStyles.side2[options.prefx + '-transform'] = 'translate3d( 0, 0,  -' + (this.size.height / 2) + 'px ) rotate3d( 1, 0, 0, -90deg )';
				this.animationStyles.side3[options.prefx + '-transform'] = 'translate3d( 0, 0,  -' + (this.size.height / 2) + 'px ) rotate3d( 1, 0, 0, -180deg )';
				this.animationStyles.side4[options.prefx + '-transform'] = 'translate3d( 0, 0,  -' + (this.size.height / 2) + 'px ) rotate3d( 1, 0, 0, -270deg )';
				...
				this.sidesStyles.frontSideStyle[options.prefx  + '-transform'] = 'rotate3d( 0, 1, 0, 0deg ) translate3d( 0, 0, ' + (this.size.height / 2) + 'px )';
				this.sidesStyles.backSideStyle[options.prefx   + '-transform'] = 'rotate3d( 0, 1, 0, 180deg ) translate3d( 0, 0, ' + (this.size.height / 2) + 'px ) rotateZ( 180deg )';
				this.sidesStyles.rightSideStyle[options.prefx  + '-transform'] = 'rotate3d( 0, 1, 0, 90deg ) translate3d( 0, 0, ' + (this.size.width / 2) + 'px )';
				this.sidesStyles.leftSideStyle[options.prefx   + '-transform'] = 'rotate3d( 0, 1, 0, -90deg ) translate3d( 0, 0, ' + (this.size.width / 2) + 'px )';
				this.sidesStyles.topSideStyle[options.prefx    + '-transform'] = 'rotate3d( 1, 0, 0, 90deg ) translate3d( 0, 0, ' + (this.size.height / 2) + 'px )';
				this.sidesStyles.bottomSideStyle[options.prefx + '-transform'] = 'rotate3d( 1, 0, 0, -90deg ) translate3d( 0, 0, ' + (this.size.height / 2) + 'px )';
			} else if (this.orientation === 'h') {
				this.animationStyles = {side1:{},side2:{},side3:{},side4:{}};
				this.animationStyles.side1[options.prefx + '-transform'] = 'translate3d( 0, 0, -' + (this.size.width / 2) + 'px )';
				this.animationStyles.side2[options.prefx + '-transform'] = 'translate3d( 0, 0,  -' + (this.size.width / 2) + 'px ) rotate3d( 0, 1, 0, -90deg )';
				this.animationStyles.side3[options.prefx + '-transform'] = 'translate3d( 0, 0,  -' + (this.size.width / 2) + 'px ) rotate3d( 0, 1, 0, -180deg )';
				this.animationStyles.side4[options.prefx + '-transform'] = 'translate3d( 0, 0,  -' + (this.size.width / 2) + 'px ) rotate3d( 0, 1, 0, -270deg )';
				...
				this.sidesStyles.frontSideStyle[options.prefx  + '-transform'] = 'rotate3d( 0, 1, 0, 0deg ) translate3d( 0, 0, ' + (this.size.width / 2) + 'px )';
				this.sidesStyles.backSideStyle[options.prefx   + '-transform'] = 'rotate3d( 0, 1, 0, 180deg ) translate3d( 0, 0, ' + (this.size.width / 2) + 'px )';
				this.sidesStyles.rightSideStyle[options.prefx  + '-transform'] = 'rotate3d( 0, 1, 0, 90deg ) translate3d( 0, 0, ' + (this.size.width / 2) + 'px )';
				this.sidesStyles.leftSideStyle[options.prefx   + '-transform'] = 'rotate3d( 0, 1, 0, -90deg ) translate3d( 0, 0, ' + (this.size.width / 2) + 'px )';
				this.sidesStyles.topSideStyle[options.prefx    + '-transform'] = 'rotate3d( 1, 0, 0, 90deg ) translate3d( 0, 0, ' + (this.size.height / 2) + 'px )';
				this.sidesStyles.bottomSideStyle[options.prefx + '-transform'] = 'rotate3d( 1, 0, 0, -90deg ) translate3d( 0, 0, ' + (this.size.height / 2) + 'px )';
		...
		createSlice : function(options, i, $imgs) {
			$slice = $('<div/>').css(this.style)
					.css(this.animationStyles.side1).append(
							$('<div/>').addClass('sb-side').css(
									this.sidesStyles.frontSideStyle)).append(
							$('<div/>').addClass('sb-side').css(
									this.sidesStyles.backSideStyle)).append(
							$('<div/>').addClass('sb-side').css(
									this.sidesStyles.rightSideStyle)).append(
							$('<div/>').addClass('sb-side').css(
									this.sidesStyles.leftSideStyle)).append(
							$('<div/>').addClass('sb-side').css(
									this.sidesStyles.topSideStyle)).append(
							$('<div/>').addClass('sb-side').css(
									this.sidesStyles.bottomSideStyle));
		...
		rotate : function(dir, i, options, $imgs, imgCurrent, callback) {
			var instance = this, classRotation, seq = (options.sequentialRotation) ? options.sequentialFactor * i : 0;

			setTimeout(function() {
				if (dir === 'next') {
					switch (instance.side) {
					case 1:
						animationStyle = instance.animationStyles.side2;
						instance.side = 2;
						break;
					case 2:
						animationStyle = instance.animationStyles.side3;
						instance.side = 3;
						break;
					case 3:
						animationStyle = instance.animationStyles.side4;
						instance.side = 4;
						break;
					case 4:
						animationStyle = instance.animationStyles.side1;
						instance.side = 1;
						break;
					default:
						animationStyle = instance.animationStyles.side1;
				...
				instance.element.css(animationStyle).animate(animateOut, options.speed3d / 2 - 50).animate(animateIn, options.speed3d / 2 - 50, function() {
					if (callback) {
	...
	$.Box.prototype = {
			...
			instance.$images.each(function(i) {
				...
				if (i === 0) {
					...
					if (options.orientation === 'v') {
						$img.css({
							left : '0px',
							top : -instance.size.height + 'px'
						});
					} else if (options.orientation === 'h') {
						$img.css({
							left : instance.size.width + 'px',
							top : '0px'
						});
					}
				}
		...
		_showInfo : function() {
			...
			var title = this.$images.eq(this.imageCurrent).attr('title');
			$('<div class="sb-title"><span>' + title + '</span></div>')
				.appendTo(this.wrapper).stop().animate({
					height : '38px',
					bottom : '0px'
				}, 300);
			this.OptionInfo.addClass('sb-nav-noinfo').removeClass('sb-nav-info');
		...
		navigate : function(dir, options) {
			...
			this._slide(dir, options, function() {
				if (options.showInfo) {
					instance._showInfo();
				}
			});
		...
		_slide : function(dir, options, callback) {
			var instance = this, $current = instance.$images.eq(instance.imageCurrent);
